...
// component for save start button
const SaveBtn = ({ data }) => {
  const { saveCoins, allCoins, deleteCoins } = useContext(StorageContext);
  const coinHandler = () => {
    let coins = JSON.parse(localStorage.getItem("coins"));
    if (coins.includes(data.id)) {
      // if the coin is already present so e call this
      deleteCoins(data.id);
    } else {
      saveCoins(data.id);
    }
  ...
  return (
    ...
    >
      <svg
        className={`w-[1.5rem] ml-2.5  hover:fill-cyan ${
...
const Saved = () => {
  ...
  const { savedCoins, resetSavedResults } = useContext(StorageContext);
      ...
      <section className="w-[80%] h-full flex flex-col mt-16 relative">
        <div className="w-full min-h-[60vh] py-8 border border-gray-100 rounded">
          {savedCoins ? (
              ...
              <tbody>
                {savedCoins.map((data) => {
                  return (
                    <tr
                      key={data.id}
                      className="cursor-pointer text-center text-base border-b border-gray-100 hover:bg-gray-200 duration-200"
                    >
                      <td className="py-4 flex items-center uppercase">
                        {/* here we have three things start icon, coin svg image, short name */}
                        <SaveBtn data={data} />

                        <img
                          src={data.image}
                          alt={data.name}
                          className="w-[1.5rem] h-[1.5rem] mx-2.5"
                        />
                        <span className="">
                          <Link to={`/saved/${data.id}`}>{data.symbol}</Link>
                        </span>
                      </td>
                      <td className="py-4">
                        <Link to={`/saved/${data.id}`}>{data.name}</Link>
                      </td>
                      ...
                      >
                        ...
                        {/* .toFixed() hamesha number data type pr use hotaa hy islye yahan hamne values ko pehly Number() data type men type cast krliaa  */}
                      </td>
                      <td
                        className={
            ...
            <h1 className="min-h-[60vh] text-xl text-cyan flex items-center justify-center">
              Empty !
            </h1>
          )}
          <button
            onClick={resetSavedResults}
